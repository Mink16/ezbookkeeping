...
func startWebServer(c *core.CliContext) error {
	...
	apiRoute.Use(bindMiddleware(middlewares.RequestLog, config))
	{
		...
		apiV1Route.Use(bindMiddleware(middlewares.APITokenIpLimit(config), config))
		{
			...
			// Upstream (fork-adopted): AI text recognition from clipboard
			...
			apiV1Route.GET("/custom/llm_prompts/list.json", bindApi(api.LlmPrompts.LlmPromptListHandler, config))
			apiV1Route.GET("/custom/llm_prompts/get.json", bindApi(api.LlmPrompts.LlmPromptGetHandler, config))
			apiV1Route.POST("/custom/llm_prompts/add.json", bindApi(api.LlmPrompts.LlmPromptCreateHandler, config))
			apiV1Route.POST("/custom/llm_prompts/modify.json", bindApi(api.LlmPrompts.LlmPromptModifyHandler, config))
			apiV1Route.POST("/custom/llm_prompts/delete.json", bindApi(api.LlmPrompts.LlmPromptDeleteHandler, config))
			apiV1Route.POST("/custom/llm_prompts/set_active.json", bindApi(api.LlmPrompts.LlmPromptSetActiveHandler, config))
			apiV1Route.POST("/custom/llm_prompts/preview.json", bindApi(api.LlmPrompts.LlmPromptPreviewHandler, config))

			// Custom (fork-local): server admins
			apiV1Route.GET("/custom/admin/status.json", bindApi(api.CustomAdmins.AdminStatusHandler, config))
			apiV1Route.GET("/custom/admin/users/list.json", bindApi(api.CustomAdmins.AdminUserListHandler, config))
			apiV1Route.POST("/custom/admin/users/grant.json", bindApi(api.CustomAdmins.AdminGrantHandler, config))
			apiV1Route.POST("/custom/admin/users/revoke.json", bindApi(api.CustomAdmins.AdminRevokeHandler, config))

			// Custom (fork-local): LLM connection profiles
			apiV1Route.GET("/custom/llm_profiles/list.json", bindApi(api.CustomLlmProfiles.ProfileListHandler, config))
			apiV1Route.GET("/custom/llm_profiles/get.json", bindApi(api.CustomLlmProfiles.ProfileGetHandler, config))
			apiV1Route.POST("/custom/llm_profiles/add.json", bindApi(api.CustomLlmProfiles.ProfileCreateHandler, config))
			apiV1Route.POST("/custom/llm_profiles/modify.json", bindApi(api.CustomLlmProfiles.ProfileModifyHandler, config))
			apiV1Route.POST("/custom/llm_profiles/delete.json", bindApi(api.CustomLlmProfiles.ProfileDeleteHandler, config))
			apiV1Route.POST("/custom/llm_profiles/set_active.json", bindApi(api.CustomLlmProfiles.ProfileSetActiveHandler, config))
			apiV1Route.POST("/custom/llm_profiles/test.json", bindApi(api.CustomLlmProfiles.ProfileTestHandler, config))

			// Custom (fork-local): LLM receipt recognition with item details and geocoding
			apiV1Route.POST("/custom/llm/transactions/recognize_receipt_image_details.json", bindApi(api.CustomReceiptRecognition.RecognizeReceiptImageDetailsHandler, config))
